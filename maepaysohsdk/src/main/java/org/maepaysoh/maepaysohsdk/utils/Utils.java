...
import org.maepaysoh.maepaysohsdk.Constants;
 ...
 */
public class Utils {
  public static void saveFontPref(MaePaySohApiWrapper.FONT font,Context context){
    SharedPreferences sdkPreferences = context.getSharedPreferences(Constants.SDK_NAME, Context.MODE_PRIVATE);
  ...
  public static boolean isUniCode(Context context) {
    SharedPreferences sdkPreferences = context.getSharedPreferences(Constants.SDK_NAME, Context.MODE_PRIVATE);
    String font = sdkPreferences.getString(Constants.FONT_STORAGE, MaePaySohApiWrapper.FONT.unicode.name());
